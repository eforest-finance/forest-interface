...
export function NFTListTable({ dataSource, elfRate }: INFTListTableProps) {
  ...
  const renderPrice = (text?: string | number) => {
    ...
    return (
      <Tooltip title={formatUSDPrice(BigNumber(elfRate).multipliedBy(BigNumber(text)))}>
  ...
  return (
    <>
      <Table
        ...
        sticky={{
          offsetHeader: isSmallScreen ? 138 : 138,
        }}
      />
