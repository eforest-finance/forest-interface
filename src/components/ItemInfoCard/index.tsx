          ...
          <div className="flex flex-1 flex-col min-h-full items-start overflow-hidden">
            {collectionName && (
              <>
                <p
                  className={clsx(
                    'text-[18px] mdTW:text-[20px] font-semibold !text-textPrimary',
                    styles['nft-list-card-text-ellipsis'],
                  )}>
                  {collectionName}
                  {number && number > 0 && (
                    <span className="inline-block text-[14px]  mdTW:text-[16px] text-brandNormal rounded-[4px] ml-[8px] !h-[24px] bg-functionalLinkBg px-[8px] line-[24px]">
                      x {number}
                    </span>
                  )}
                </p>
            ...
            <p
              className={clsx(
                'text-base font-medium text-textSecondary mt-[8px] mdTW:mt-[16px]',
                styles['nft-list-card-text-ellipsis'],
              )}>
              {nftName}
            </p>
        ...
        {!isSmallScreen ? (
          <>
            <div className="flex min-w-fit flex-col justify-center items-end">
              ...
              {price && <p className="mt-[4px] text-xl font-semibold text-textPrimary">{price}</p>}
              {usdPrice && <p className="mt-[4px] text-base font-medium text-textSecondary">{usdPrice}</p>}
            </div>
          </>
        ) : (
          <div className="flex min-w-fit justify-between ">
            ...
            <div>
              {price && <div className="text-right text-[18px] font-semibold text-textPrimary">{price}</div>}
